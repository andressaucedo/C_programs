...
# %ecx    # holds the number of objects to run through
#
...
movl $13, %ecx    # assign an index of 13 to a register ecx
leal (,%ecx,4), ecx     # multiply ecx by wordsize 

start_loop:       # start program loop
  cmpl %edi ,%ecx     # check to see if we've hit the end by
                               # multiplying ecx by word size
  je loop_exit
